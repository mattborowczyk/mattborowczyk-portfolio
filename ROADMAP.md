...
**Sanity v3** (embedded Studio at `/studio`) · **MailerLite** (newsletter) ·
**Easytools** (planned course checkout) · **Shopify** (planned shop).
...
- [ ] Add a real `README.md` (project intro, local-dev instructions, env vars)
- [ ] Document required env vars (Sanity project ID/dataset/token, MailerLite key/group)
- [ ] Confirm `npm install && npm run build` passes cleanly before further work
...
- [ ] Install `@portabletext/react` and **render the About page body** (currently
      a placeholder — the About content does not display)
- [ ] Add **Collections index page** (`/collections`) — detail pages exist but
      there is no listing page; `allCollectionsQuery` is defined but unused
- [ ] Model **courses & digital products in Sanity** instead of hardcoding them
      in `courses/page.tsx`
- [ ] Add a content type / section for the **Goldsmith Studio app** and resources
      (brushes, presets, software access)
- [ ] Replace deprecated `__experimental_actions` singleton pattern in
      `settings` schema with proper singleton document actions
...
- [ ] **Draft/Preview mode**: `SANITY_API_TOKEN` is in env but never used and
      there's no preview wiring. Either implement Next.js draft mode + a preview
      client (`useCdn:false`, `perspective: drafts`) or remove the token
- [ ] Fix the `sanityFetch` **tag/revalidate logic** — when `tags` are passed it
      sets `revalidate:false`, so tagged content never updates without a webhook
- [ ] Add a **`/api/revalidate` webhook route** wired to Sanity webhooks for
      on-publish revalidation (`revalidateTag`)
- [ ] **Deduplicate** Sanity fetches per request with React `cache()` (the footer
      and pages refetch `settingsQuery` separately)
